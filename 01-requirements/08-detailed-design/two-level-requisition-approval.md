...
Feature นี้เป็นแกนกลางของ Epic 1: เภสัชกรผู้อนุมัติระดับ 1 พิจารณา/ปฏิเสธ/ปรับจำนวนคำขอ จากนั้นเภสัชกรผู้ตรวจสอบระดับ 2 (คนละคนกับระดับ 1 เสมอ — บังคับที่ระดับ operation) ยืนยันซ้ำก่อนสถานะเปลี่ยนเป็น "อนุมัติแล้ว" → "พร้อมส่งออก" โดยอัตโนมัติ ตาม [pharmacist-journey.md](../03-user-journey/pharmacist-journey.md) step 4-13 (แก้ไข 20260915 — เดิมอ้าง "step 3-9" ซึ่งตกยุคแล้วหลัง pharmacist-journey.md ถูกเลื่อนหมายเลข step หลายรอบตั้งแต่ 20260824 เป็นต้นมา รวมถึงรอบล่าสุด 20260914 ที่แทรก step ใหม่ "เรียกดูผู้ช่วย AI"; ช่วงที่ถูกต้องตรงกับเนื้อหาของ Feature นี้คือ step 4 "เห็นคำขอเบิกที่รอการอนุมัติระดับ 1" ถึง step 13 "สถานะเปลี่ยนเป็น อนุมัติแล้ว / พร้อมส่งออก" ในไฟล์เวอร์ชันปัจจุบัน)
...
- **20260915 (audit, ระหว่างสร้าง [pharmacist-approval-ai-assistant.md](pharmacist-approval-ai-assistant.md) สำหรับ FT-036 ใหม่ — ตรวจสอบ cross-reference ที่ถูกตั้งข้อสังเกตไว้ว่าอาจตกยุค):** ยืนยันแล้วว่าอ้างอิง "step 3-9" ในหัวข้อ 1 **ตกยุคจริง** — เทียบกับ [pharmacist-journey.md](../03-user-journey/pharmacist-journey.md) เวอร์ชันปัจจุบันบนดิสก์โดยตรง (ไม่ใช่แค่เชื่อรายงานเดิม) พบว่าไฟล์ journey ถูกเลื่อนหมายเลข step หลายรอบตั้งแต่สร้างไฟล์นี้ครั้งแรก (20260823/20260824 ที่เพิ่ม Login/OTP/Concurrency Check และล่าสุด 20260914 ที่แทรก step ใหม่ "เรียกดูผู้ช่วย AI" ระหว่าง step เดิม) ช่วง step ที่ตรงกับเนื้อหาของ Feature นี้จริง (จาก "เห็นคำขอเบิกที่รอการอนุมัติระดับ 1" ถึง "สถานะเปลี่ยนเป็น อนุมัติแล้ว / พร้อมส่งออก") คือ **step 4-13** ในเวอร์ชันปัจจุบัน ไม่ใช่ step 3-9 — แก้ไขอ้างอิงในหัวข้อ 1 พร้อมหมายเหตุประวัติสั้นๆ ว่าทำไมถึงตกยุค — ไม่แก้ไดอะแกรม 2.1-2.4 หรือ state diagram หัวข้อ 3 (ไม่มีเนื้อหาใดใน BL-004/BL-005 เปลี่ยนแปลงจากการเลื่อนหมายเลข step ของ journey เอง เป็นแค่การอ้างอิงเลขลำดับที่ต้อง sync ตาม)
