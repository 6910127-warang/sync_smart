...
ระบบแจ้งเตือนเจ้าหน้าที่ รพ.สต. ล่วงหน้าก่อนถึงกำหนดส่งคำขอเบิกประจำเดือน เพื่อลดปัญหา "ลืมเบิกยา" ซึ่งเป็นหนึ่งใน Root Cause ของการเบิกด่วนนอกรอบ ตาม [staff-hph-journey.md](../03-user-journey/staff-hph-journey.md) step 11 — **จำนวนวันล่วงหน้าที่แน่นอนยัง `[รอยืนยัน]`** (BL-009b) จึงแสดงเป็นค่าตัวแปรในไดอะแกรม ไม่ระบุตัวเลขที่ยังไม่ได้ยืนยัน
...
sequenceDiagram
    ...
    REQ->>NOTIFY: ถึงจำนวนวันที่กำหนดไว้ล่วงหน้า `[รอยืนยัน — จำนวนวันที่แน่นอน]`
...
- `[รอยืนยัน]` จำนวนวันล่วงหน้าที่ควรแจ้งเตือนก่อนกำหนด — สืบทอดจาก backlog.md/spec 002 ไม่ใช่ประเด็นใหม่ของไฟล์นี้ ต้องยืนยันกับเภสัชกรเจ้าของระบบก่อนกำหนดค่าจริง
