...
flowchart TD
    ...
    H --> H2["ดาวน์โหลดใบเบิกยาแบบพิมพ์ (PDF) เพื่อเซ็นชื่อ"]
    H2 --> I["ยืนยันรับยาเข้าคลังของหน่วยตนเอง"]
...
4. **ระบบเปรียบเทียบยอดที่แจ้งเองกับยอดคงคลังที่คำนวณ (แจ้งเตือนทันทีถ้าไม่ตรงกัน)** — หลังกรอกยอดคงเหลือ ระบบเปรียบเทียบยอดคงเหลือที่เจ้าหน้าที่แจ้งเอง (FR-1.1a) กับยอดคงคลังที่ระบบคำนวณอัตโนมัติ (FR-1.5) ของหน่วยเดียวกันในช่วงเวลาเดียวกัน เมื่อพบว่าไม่ตรงกัน (ไม่ว่าจะต่างกันเท่าใด — เปรียบเทียบแบบ exact-match/zero-tolerance) ระบบแสดงข้อความแจ้งเตือนทันที: "ยอดคงเหลือที่เจ้าหน้าที่แจ้งกับยอดคงคลังที่ระบบคำนวณอัตโนมัติ ไม่ตรงกัน โปรดตรวจสอบ หากไม่แน่ใจให้ปรึกษา รพศ.ชร. (แม่ข่าย)" — ยืนยันครบแล้ว 20260823 (รอบ 4) การตรวจสอบ/คอนเฟิร์มความคลาดเคลื่อนนี้เป็นหน้าที่ของ**เภสัชกร** (ไม่ใช่เจ้าหน้าที่ รพ.สต. ที่เห็นข้อความแจ้งเตือนในหน้าจอนี้) และเกิดขึ้นในภายหลังระหว่างขั้นตอนอนุมัติระดับ 1 (ดู [pharmacist-journey.md](pharmacist-journey.md) step 8) โดยเภสัชกรจะเป็นผู้กรอกยอดคงเหลือที่ถูกต้องเอง ซึ่งกลายเป็นค่าที่มีผลผูกพันต่อแทนยอดที่เจ้าหน้าที่รายงานไว้ในขั้นตอนนี้ (อ้างอิง FR-1.1a, FR-1.5, FR-1.9, FR-1.9a, FR-1.9b, US-1.6, BL-032, BL-004)
...
6. **ยืนยันคำขอเบิก (รอการอนุมัติระดับ 1)** — เจ้าหน้าที่กดยืนยันคำขอ ระบบบันทึกคำขอสถานะ "รอการอนุมัติระดับ 1" (อ้างอิง FR-1.1a, US-1.1, BL-002) — ชื่อของเจ้าหน้าที่และวันเวลาที่ยืนยันคำขอนี้จะถูกพิมพ์ไว้ล่วงหน้าในบล็อกลายเซ็น "ผู้เบิก" ของใบเบิกยาแบบพิมพ์ (PDF) ที่ดาวน์โหลดได้ภายหลัง (ดู step 9 ด้านล่าง, อ้างอิง FR-7.5, BL-046)
...
9. **ดาวน์โหลดใบเบิกยาแบบพิมพ์ (PDF) เพื่อเซ็นชื่อ** — เมื่อคำขอผ่านอนุมัติครบ 2 ระดับแล้ว (เงื่อนไขจังหวะเดียวกับการดาวน์โหลดไฟล์ Excel ใน step 8) เจ้าหน้าที่ยังดาวน์โหลด**ใบเบิกยาและเวชภัณฑ์แบบพิมพ์**ในรูปแบบไฟล์ PDF ได้อีกช่องทางหนึ่ง เพื่อนำไปเซ็นชื่อจริงและส่งต่อเป็นหลักฐานยืนยันการเบิก-จ่ายกับหน่วยงานอื่น (ผอ.รพ.สต. และเภสัชกรผู้จ่ายของที่คลังยา รพ.แม่ข่าย) — **แยกต่างหากโดยสิ้นเชิงจากไฟล์ Excel นำเข้า INVC ใน step 8** (อ้างอิง FR-7.1, US-7.1, BL-044) ระบบแยกไฟล์ตามคลังต้นทาง (เบิกตึกผลิต/เบิกคลังใต้ดิน) คูณหมวดยา (ปกติ/บัญชีแพทย์) แบบ cross-product เต็มรูปแบบ สูงสุด 4 ไฟล์ต่อคำขอ สร้างเฉพาะไฟล์ที่มีรายการยาจริง (อ้างอิง FR-7.2, FR-7.2a, US-7.2, BL-045) แต่ละไฟล์มีบล็อกลายเซ็น 4 ตำแหน่ง — "ผู้เบิก" และ "ผู้ตรวจสอบ" พิมพ์ชื่อ+วันเวลาไว้ล่วงหน้าใต้เส้น (ดึงจากการยืนยันคำขอของเจ้าหน้าที่เองใน step 6 และการอนุมัติระดับ 1 ของเภสัชกร — ดู [pharmacist-journey.md](pharmacist-journey.md) step 6) แต่ยังต้องเซ็นจริงบนเส้นด้วย ส่วน "ผู้รับรอง" (ผอ.รพ.สต.) และ "ผู้จ่ายของ" (เภสัชกรคลังยา รพ.แม่ข่าย) เว้นเส้นว่างเปล่า **ไม่มี role ในระบบ SmartSync และไม่มีการสร้าง role ใหม่ใดๆ เพื่อรองรับ** เซ็นชื่อเองนอกระบบทั้งหมด (อ้างอิง FR-7.5, FR-7.6, FR-7.7, FR-7.8, US-7.3, BL-046) ช่อง "วันที่รับของ" ท้ายฟอร์มพิมพ์วันที่ไว้ล่วงหน้าถ้าเจ้าหน้าที่ยืนยันรับยาแล้ว (step 10 ด้านล่าง) ก่อนเวลาที่สร้าง/ดาวน์โหลดไฟล์นี้ มิฉะนั้นเว้นว่างให้กรอกด้วยมือ (อ้างอิง FR-7.10, BL-044)

10. **ยืนยันรับยาเข้าคลังของหน่วยตนเอง** — เมื่อยาถูกจ่ายจาก รพ.แม่ข่ายแล้ว เจ้าหน้าที่ยืนยันการรับยาในระบบ ทำให้ยอดคงคลังของหน่วยอัปเดตทันที (อ้างอิง FR-1.4, US-1.3, BL-006)

11. **ดูยอดคงคลังของหน่วยตนเอง (Real-time)** — เจ้าหน้าที่เห็นยอดคงคลังของหน่วยตนเองอัปเดตภายใน 5 วินาทีหลังมีการบันทึกรับ/จ่าย เพื่อวางแผนเบิก-จ่ายได้แม่นยำ (อ้างอิง FR-1.5, BL-007)

12. **ได้รับแจ้งเตือนยาใกล้หมด / แจ้งเตือนล่วงหน้าก่อนกำหนดเบิก** — ระบบแจ้งเตือนเจ้าหน้าที่เมื่อยอดคงคลังต่ำกว่าเกณฑ์ safety stock (อ้างอิง FR-2.2, US-2.1, BL-013) และแจ้งเตือนล่วงหน้าก่อนถึงกำหนดส่งคำขอเบิกประจำเดือนเพื่อลดปัญหาลืมเบิกยา (อ้างอิง FR-1.8, US-1.5, BL-009b) ทั้งสองผ่านช่องทางอีเมล+LINE OA (อ้างอิง FR-2.3, BL-014) วนกลับเข้าสู่รอบเบิกถัดไป

13. **ยาหมดกะทันหันนอกรอบเดือน → สร้างคำขอเบิกฉุกเฉิน** — กรณียาหมดกะทันหันนอกรอบเดือนปกติ เจ้าหน้าที่สร้างคำขอเบิกฉุกเฉิน (Off-cycle) ซึ่งเข้าสู่ผังอนุมัติสองระดับเดียวกับคำขอปกติแต่มี notify ทันทีให้ผู้อนุมัติทั้งสองระดับ (อ้างอิง FR-1.7, US-1.4, BL-009)
...
- **20260831 (audit, stage 2 ของ /check-backlog-sync, หลัง backlog-sync-checker เพิ่ม spec 007/BL-044-046, FT-035 ใหม่ใน Feature List):** ตรวจสอบไฟล์นี้เทียบกับ backlog.md เวอร์ชันล่าสุด พบว่า BL-044/BL-045/BL-046 (ใบเบิกยาแบบพิมพ์เพื่อเซ็นชื่อ) ยังไม่ปรากฏใน journey นี้ ทั้งที่เจ้าหน้าที่ รพ.สต. เป็นผู้ดาวน์โหลดโดยตรงตาม FR-7.1/FR-7.12 — เพิ่ม node ใหม่ `H2` ในไดอะแกรมระหว่าง "ดาวน์โหลดไฟล์ Excel" (H) กับ "ยืนยันรับยา" (I เดิม) และเพิ่มคำอธิบายเป็น step ใหม่ที่ 9 (เลื่อนหมายเลข step เดิม 9-12 เป็น 10-13) ครอบคลุมเงื่อนไขดาวน์โหลด, การแยกไฟล์ cross-product สูงสุด 4 ไฟล์, บล็อกลายเซ็น 4 ตำแหน่ง, และช่อง "วันที่รับของ" พร้อมย้ำชัดเจนว่า**ไม่มี role ใหม่ใดๆ ถูกสร้างขึ้นในระบบสำหรับ "ผู้รับรอง"/"ผู้จ่ายของ"** — เพิ่มเติม (enrichment คู่กัน): step 6 (ยืนยันคำขอเบิก) เพิ่มหมายเหตุ cross-reference ว่าชื่อ+เวลาที่ยืนยันนี้ถูกพิมพ์ล่วงหน้าในบล็อกลายเซ็น "ผู้เบิก" ของ step 9 ใหม่ — ตรวจสอบส่วนที่เหลือของไฟล์ไม่พบจุดอื่นที่ต้องแก้ไข (auto-fixed ทั้งหมด มีหลักฐานตรงตัวจาก backlog.md/spec 007 ไม่กำกวม) — ดู [pharmacist-journey.md](pharmacist-journey.md) สำหรับ enrichment คู่กันของ step อนุมัติระดับ 1
- **20260914 (audit, stage 2 ของ /check-backlog-sync, หลัง backlog-sync-checker ยืนยัน spec 008/BL-047 sync ครบ):** ตรวจสอบไฟล์นี้เทียบกับ backlog.md เวอร์ชันล่าสุด — **BL-047** (ผู้ช่วย AI สำหรับเภสัชกรผู้อนุมัติระดับ 1) เป็นการกระทำของเภสัชกรเท่านั้น เจ้าหน้าที่ รพ.สต. ไม่มีปฏิสัมพันธ์กับความสามารถนี้เลย จึง**ไม่เพิ่ม step/node ใหม่**ในไฟล์นี้ — แต่การเพิ่ม step ใหม่ใน [pharmacist-journey.md](pharmacist-journey.md) (step 7 ใหม่) ทำให้หมายเลข step เดิมของไฟล์นั้นเลื่อนไปหมด ส่งผลให้ step 4 ของไฟล์นี้ที่เคยอ้างอิง "pharmacist-journey.md step 7" (ขั้นตอนกระทบยอด BL-032) ต้องแก้เป็น**step 8**แทน — แก้ cross-reference นี้ให้ตรง (auto-fixed, มีหลักฐานตรงตัวจากการเลื่อนหมายเลขใน pharmacist-journey.md ไม่กำกวม) — ตรวจสอบ cross-reference อื่นที่เหลือของไฟล์นี้ (step 9 อ้างอิง "pharmacist-journey.md step 6") ไม่กระทบ เนื่องจาก step 6 ของ pharmacist-journey.md ไม่เปลี่ยนตำแหน่ง — ไม่พบจุดอื่นในไฟล์นี้ที่ต้องแก้ไข
