...
> **(resolved 20260916)** ฟิลด์เพิ่มเติมอื่นที่อาจจำเป็นในเชิงคลินิก เช่น รูปแบบยา (dosage form), หน่วยนับ (unit of measure), กลุ่มยาทางการแพทย์ — เจ้าของระบบยืนยันผ่าน `AskUserQuestion` ในการสนทนาหลักว่า**ยังไม่ต้องเพิ่มในรอบนี้** — เป็นการตัดสินใจแล้วว่าไม่ทำตอนนี้ (resolved) ไม่ใช่การปฏิเสธถาวร หากในอนาคตมีความจำเป็นต้องเพิ่ม ต้องถามเภสัชกรเจ้าของระบบใหม่อีกครั้งก่อนเพิ่ม (แยกจาก 3 ฟิลด์ WORKING_CODE/HOSP_CODE/หมวดบัญชีแพทย์ ที่ยืนยันแล้วด้านบน ซึ่งไม่ใช่ฟิลด์คลินิก)
...
- ~~**[สืบทอดจาก spec 008/05-architecture.md]** กลไก PDPA แบบละเอียด (point-of-use consent/notice) สำหรับกรณีที่ข้อมูลคำขอเบิก/ประวัติหน่วยงานถูกส่งออกนอกระบบไปยังผู้ให้บริการ AI ภายนอกจริงในทางเทคนิค (BL-047/FT-036)~~ — **resolved 20260916** (สืบทอดจาก spec 008/05-architecture.md ที่ปิดในวันเดียวกัน): ข้อความแจ้งสั้นๆ ข้างปุ่มเรียกใช้ผู้ช่วย AI — ไม่กระทบโครงสร้าง entity ใดๆ ในเอกสารนี้เช่นเดิม (ไม่มีการวาด entity/องค์ประกอบสำหรับผู้ให้บริการ AI ภายนอก ดู [05-architecture.md](05-architecture.md) หัวข้อ 6/8)
- ~~`[ถือว่า]` สิทธิ์การดาวน์โหลดใบเบิกยาแบบพิมพ์ (PDF) ครบทุก role หรือไม่ (FR-7.12, [spec 007](01-spec/20260831-007-printable-signed-requisition-form.md))~~ — **resolved 20260916**: เจ้าของระบบยืนยันผ่าน `AskUserQuestion` ในการสนทนาหลักว่าสิทธิ์การดาวน์โหลด = เจ้าหน้าที่ รพ.สต. เจ้าของคำขอ (เดิม) + เภสัชกร (ผู้อนุมัติระดับ 1/2 ทุกคน) + ผู้บริหาร รพ./สสอ. (executive) + ผู้ดูแลระบบ (Admin) — ไม่กระทบโครงสร้าง entity ของเอกสารนี้ (เป็นประเด็นระดับสิทธิ์การเข้าถึง สะท้อนแล้วใน [07-api-spec.md](07-api-spec.md) หัวข้อ 3.12/5)
...
- ~~`[รอยืนยัน]` ฟิลด์เพิ่มเติมของ "รายการยา (Drug/Item Master)" ในเชิงคลินิก (รูปแบบยา/หน่วยนับ/กลุ่มยาทางการแพทย์)~~ — **resolved 20260916**: เจ้าของระบบยืนยันผ่าน `AskUserQuestion` ในการสนทนาหลักว่ายังไม่ต้องเพิ่มฟิลด์เชิงคลินิกในรอบนี้ — เป็นการตัดสินใจแล้วว่าไม่ทำตอนนี้ ไม่ใช่การปฏิเสธถาวร (แยกจาก 3 ฟิลด์ WORKING_CODE/HOSP_CODE/หมวดบัญชีแพทย์ ที่ยืนยันแล้วในหัวข้อ 3.3 — ฟิลด์เหล่านั้นไม่ใช่ฟิลด์คลินิก) ดูหัวข้อ 3.3
...
- **20260916 (ปิด Open Items 3 ข้อที่ผู้ประสานสำรวจทั้งโปรเจกต์และเจ้าของระบบตอบผ่าน `AskUserQuestion` ในการสนทนาหลัก, คู่กับการปิดเดียวกันใน [07-api-spec.md](07-api-spec.md), [spec 007](01-spec/20260831-007-printable-signed-requisition-form.md), [spec 008](01-spec/20260914-008-pharmacist-approval-ai-assistant.md), และ [05-architecture.md](05-architecture.md)):** แก้ไข 3 จุด — (1) หัวข้อ 6 ปิด `[ถือว่า]` สิทธิ์การดาวน์โหลดใบเบิกยาแบบพิมพ์ (PDF, FR-7.12) เป็นยืนยันแล้ว: เจ้าหน้าที่ รพ.สต. เจ้าของคำขอ + เภสัชกร (ระดับ 1/2 ทุกคน) + ผู้บริหาร + ผู้ดูแลระบบ (Admin) — ไม่กระทบโครงสร้าง entity ใดๆ (2) หัวข้อ 3.3/6 ปิด `[รอยืนยัน]` ฟิลด์เชิงคลินิกเพิ่มเติมของ DrugItem เป็นยืนยันแล้วว่า**ยังไม่ต้องเพิ่มในรอบนี้** (resolved-not-now ไม่ใช่การปฏิเสธถาวร) — ไม่เพิ่มฟิลด์ใหม่ใดๆ ใน entity นี้ (3) หัวข้อ 6 ปิดรายการ open item PDPA point-of-use consent ที่สืบทอดจาก spec 008 เป็น resolved ด้วยเช่นกัน (เดิมเป็นเพียงหมายเหตุ cross-reference "ไม่ใช่ open item ใหม่ของไฟล์นี้" แต่ยังค้างสถานะเปิดอยู่ทั้งที่ต้นทาง — spec 008/05-architecture.md — ปิดแล้วในรอบเดียวกันนี้) — ไม่พบ drift อื่นเพิ่มเติมในขอบเขตที่ตรวจสอบรอบนี้
